...
class QueryFrameMatcher:
    ...
    def compare_frame_hashes_rgb(self, frame_data1, frame_data2, width=352, height=288):
        ...
        frame2 = np.frombuffer(frame_data2, dtype=np.uint8).reshape((height, width, 3))
    ...
    def get_rgb_frame_match(self, rgb_path, frame_num, query_path, query_frame_count, start_time):
        ...
        query_path_parts = query_path_parts[:-2] + ['Queries/RGBs'] + [query_path_parts[-1].split('.')[0] + '.rgb']
        ...
        while end_frame_number <= max_end_frame_number:
            start_video_frame = self.get_frame_data(rgb_path, start_frame_number)
    ...
    def find_query(self, query_path, matched_chunk):
        ...
        exact_time_rgb = self.get_rgb_frame_match(os.path.join(self.rgbs_dir, matched_chunk['video'].split('.')[0] + '.rgb'), 0, query_path, frame_count, start_time)
